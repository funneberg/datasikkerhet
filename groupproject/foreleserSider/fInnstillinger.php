<?php
  ...
  //Brukerautentisering
	...
	$studentabell = "SELECT count(*) as cntStudent from student where studentID = '$username'";
	$resultat1 = mysqli_query($con,$studentabell);
	$row1 = mysqli_fetch_array($resultat1);
	
	//Sjekker igjennom ansatt tabellen:
	$ansatttabell = "SELECT count(*) as cntForeleser from foreleser where ansattID = '$username'";
  ...
  if(isset($_GET['byttPassord'])) {
    ...
    $sjekk = " SELECT passord FROM forelesere WHERE passord = '$gammeltPassord'  AND '$username' = ansattID ";

    if(mysqli_query( $con, $sjekk )){

      $sql = "UPDATE `forelesere` SET `passord`= '$passord1' WHERE ansattID = '$username'";
      ...
      echo "ERROR: Could not able to execute $sjekk. " . mysqli_error($con);
    }
